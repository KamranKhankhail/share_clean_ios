...
final class PhotoLibraryManager {
    ...
    private func getOrCreateAlbum(named: String) async -> PHAssetCollection? {
        ...
        await PHPhotoLibrary.shared().performChanges { placeholder = PHAssetCollectionChangeRequest.creationRequestForAssetCollection(withTitle: named).placeholderForCreatedAssetCollection }
